...
def _config():
    host=os.getenv('LDAP_HOST','').strip(); base=(os.getenv('LDAP_BASE') or os.getenv('LDAP_BASE_DN','')).strip(); bind_dn=os.getenv('LDAP_BIND_DN','').strip(); password=os.getenv('LDAP_BIND_PASSWORD') or os.getenv('LDAP_PASS',''); uid=(os.getenv('LDAP_UID') or os.getenv('LDAP_UID_ATTRIBUTE','sAMAccountName')).strip(); port=int(os.getenv('LDAP_PORT','389'))
    ...
    return host,port,base,bind_dn,password,uid
...
def authenticate(username,password):
    ...
    host,port,base,bind_dn,bind_password,uid=_config(); server=Server(host,port=port,use_ssl=(port==636),connect_timeout=8)
    service=Connection(server,user=bind_dn,password=bind_password,read_only=True,auto_bind=False)
    try:
        ...
        entry=service.entries[0]; attrs=entry.entry_attributes_as_dict; check=Connection(server,user=str(entry.entry_dn),password=password,read_only=True,auto_bind=False)
        try:
            if not check.bind(): raise ValueError('invalid LDAP credentials')
        finally: check.unbind()
        first=lambda key:str((attrs.get(key) or [''])[0])
        return LDAPIdentity(username=username,dn=str(entry.entry_dn),display_name=first('displayName'),email=first('mail'))
